...
# TZB 2022.1.29 随机获取两个交叉点，修改为，单点交叉，且指定位置
class PMXCrossover_TZB(Crossover[PermutationSolution, PermutationSolution]):
    def __init__(self, probability: float):
        super(PMXCrossover_TZB, self).__init__(probability=probability)
    ...
    def execute(self, parents: List[PermutationSolution]) -> List[PermutationSolution]:
        ...
        if rand <= self.probability:
            ...
            cross_points = [0, random.randint(1, permutation_length)]

            # Is element repeated in "collection"?
            # repeated: return 1; unrepeated: return 0
    ...
    def get_name(self):
        return 'Partially Matched crossover (TZB)'
...
class IntegerDifferentialEvolutionCrossover(Crossover[IntegerSolution, IntegerSolution]):
    """ This operator receives two parameters: the current individual and an array of three parent individuals. The
    best and rand variants depends on the third parent, according whether it represents the current of the "best"
    individual or a random_search one. The implementation of both variants are the same, due to that the parent selection is
    external to the crossover operator.
    ...
    def __init__(self, CR: float, F: float, K: float = 0.5):
        super(IntegerDifferentialEvolutionCrossover, self).__init__(probability=1.0)
        self.CR = CR
        self.F = F
        self.K = K
    ...
    def get_name(self) -> str:
        return 'Float Differential Evolution crossover'
...
class BinaryDifferentialEvolutionCrossover(Crossover[BinarySolution, BinarySolution]):
    """ This operator receives two parameters: the current individual and an array of three parent individuals. The
    best and rand variants depends on the third parent, according whether it represents the current of the "best"
    individual or a random_search one. The implementation of both variants are the same, due to that the parent selection is
    external to the crossover operator.
    ...
    def __init__(self, CR: float, F: float, K: float = 0.5):
        super(BinaryDifferentialEvolutionCrossover, self).__init__(probability=1.0)
        self.CR = CR
        self.F = F
        self.K = K
    ...
    def execute(self, parents: List[BinarySolution]) -> List[BinarySolution]:
        ...
        # number_of_bits = parents[0].get_total_number_of_bits()
        # rand = random.randint(0, number_of_bits - 1)
        ...
        child = parents[2] and parents[0] or parents[1]
    ...
    def get_name(self) -> str:
        return 'Binary Differential Evolution crossover'
