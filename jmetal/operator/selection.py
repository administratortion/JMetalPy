...
class BinaryTournamentSelection(Selection[List[S], S]):  # 种群中随机选两个，比较后挑个好的
...
class DifferentialEvolutionSelection_best_1(Selection[List[S], List[S]]):
    """
    DE/best/1
    """

    def __init__(self):
        super(DifferentialEvolutionSelection_best_1, self).__init__()
        self.front_size = None
    ...
    def execute(self, solutions: List[S]) -> List[S]:
        if solutions is None:
            ...
            raise Exception('The solutions has less than four solutions: ' + str(len(solutions)))

        # solutions, 进入算法的所有解列表
        # front_size, 当前父代中前沿解的个数
        solutions_size = len(solutions)
        if self.front_size <= int(0.6 * solutions_size):
            lower_bound_index_23 = self.front_size
        else:
            lower_bound_index_23 = 0

        selected_index_1 = random.sample(range(self.front_size), 1)
        selected_index_23 = random.sample(range(lower_bound_index_23, solutions_size), 2)
        while selected_index_1 in selected_index_23:
            selected_index_1 = random.sample(range(self.front_size), 1)
            selected_index_23 = random.sample(range(lower_bound_index_23, solutions_size), 2)

        selected_indexes = selected_index_1
        selected_indexes += selected_index_23

        return [solutions[i] for i in selected_indexes]

    def set_front_size(self, front_size: int):
        self.front_size = front_size

    def get_name(self) -> str:
        return "Differential evolution selection best_1"
