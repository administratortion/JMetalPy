...
# TZB 2021.10.31 generate_summary_from_experiment()中，增加“归一化”
...
from jmetal.core.quality_indicator import QualityIndicator, InvertedGenerationalDistance
...
class Job:
    ...
    def execute(self, output_path: str = ''):
        ...
        if output_path:
            """
            jMetalPy Original：输出最后一代的全部种群解
            """
            ...
            front = get_non_dominated_solutions(self.algorithm.get_result())    # 求pf

            file_name=os.path.join(output_path, 'FUN.{}.tsv'.format(self.run_tag))
            print_function_values_to_file(front, filename=file_name)

            file_name=os.path.join(output_path, 'VAR.{}.tsv'.format(self.run_tag))
            print_variables_to_file(front, filename=file_name)
                    ...
                    if hasattr(indicator, 'reference_front'):
                        if Path(reference_front_file).is_file():
                            with open(reference_front_file) as file:
                                ...
                                # normalization start  (TZB 2021.10.31)
                                num_obj = len(reference_front[0])  # 目标数
                                min_max = [[] for i in range(num_obj)]  # 存入真值目标的min/max值
                                len_reference_front = len(reference_front)
                                for i in range(num_obj):
                                    temp_data = []
                                    for j in range(len_reference_front):
                                        temp_data.append(reference_front[j][i])
                                    temp_data.sort()
                                    min_max[i].append(temp_data[0])  # min
                                    min_max[i].append(temp_data[len_reference_front-1]) # max

                                for i in range(num_obj):
                                    for j in range(len_reference_front):
                                        reference_front[j][i] = (reference_front[j][i]-min_max[i][0])/(min_max[i][1]-min_max[i][0])
                                    for k in range(len(solutions)):
                                        solutions[k].objectives[i] = (solutions[k].objectives[i]-min_max[i][0])/(min_max[i][1]-min_max[i][0])
                                # normalization end
