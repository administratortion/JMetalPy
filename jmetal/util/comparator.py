...
class OverallConstraintViolationComparator(Comparator):
    def compare(self, solution1: Solution, solution2: Solution) -> int:
        violation_degree_solution_1 = overall_constraint_violation_degree(solution1)  # 需要自定义
        violation_degree_solution_2 = overall_constraint_violation_degree(solution2)
        if violation_degree_solution_1 < 0 and violation_degree_solution_2 < 0:  # 若两个解都有违反约束
            if violation_degree_solution_1 < violation_degree_solution_2:  # 若solution1违反的少
                result = -1
            elif violation_degree_solution_2 < violation_degree_solution_1:  # 若solution2违反的少
                result = 1
            else:
                result = 0
...
class DominanceComparator(Comparator):
    ...
    def compare(self, solution1: Solution, solution2: Solution) -> int:
        ...
        # 先比约束违反情况，solution1好，返回1; solution2好，返回-1；一样，返回0
    ...
    # TZB 1支配2，返回1；2支配1，返回-1，互不支配，返回0
    @staticmethod
