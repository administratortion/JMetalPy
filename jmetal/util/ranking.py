...
class FastNonDominatedRanking(Ranking[List[S]]):
    ...
    def compute_ranking(self, solutions: List[S], k: int = None):
        ...
        """
        # number of solutions dominating solution ith 支配
        dominating_ith = [0 for _ in range(len(solutions))]

        # list of solutions dominated by solution ith 被支配
        ...
        for p in range(len(solutions) - 1):
            for q in range(p + 1, len(solutions)):
                ...
                if dominance_test_result == -1:
                    ith_dominated[p].append(q)  # 被p支配的列表追加q
                    dominating_ith[q] += 1  # 支配q的解数量累加
                elif dominance_test_result == 1:
                    ith_dominated[q].append(p)  # 被q支配的列表追加p
                    dominating_ith[p] += 1  # 支配p的解数量累加
        ...
        # 通过各级front[]中存储解的索引号，把对应的solution查出来，存入ranked_sublists[]
        self.ranked_sublists = [[]] * i   # i是前面while停止时的值，表示分级数
        for j in range(i):
            q = [0] * len(front[j])
