...
from jmetal.util.solution import print_function_values_to_file, get_non_dominated_solutions
...
class Algorithm(Generic[S, R], threading.Thread, ABC):
    ...
    def run(self) -> object:
        ...
        from jmetal.util.solution import print_variables_to_file, print_stamp_to_file
        # print_stamp_to_file('VAR.' + algorithm_name + '.MOQMKP_PER', mode = 'w', stamp = '---Init_Solutions---')
        # print_variables_to_file(self.solutions, 'VAR.' + algorithm_name + '.MOQMKP_PER', mode = 'a+')
        front = get_non_dominated_solutions(self.solutions)
        # print_stamp_to_file('FUN.' + algorithm_name + '.MOQMKP_PER', mode = 'w', stamp = '---Init_Solutions---')
        # print_function_values_to_file(self.solutions, 'FUN.' + algorithm_name  + '.MOQMKP_PER', mode = 'a+')

        # 画散点图
        # from jmetal.util.solution import get_non_dominated_solutions
        # import jmetal.lab.visualization
        # plot_front = jmetal.lab.visualization.Plot(title='Pareto front approximation', axis_labels=['x', 'y'])

        LOGGER.debug('Running main loop until termination criteria is met')
        g=0
        while not self.stopping_condition_is_met():
            self.step()             # 进化过程，包括selection、交叉、变异、replace
            self.update_progress()  # 从算法中读出一些更新量，包括评价次数、get当前解等

            g = g+1

            # 画散点图
            # front = get_non_dominated_solutions(self.get_result())
            # plot_front.plot(front, label='NSGAII-MOKP', filename='NSGAII-MOKP', format='png')

            # save to files
            # print_stamp_to_file('VAR.' + algorithm_name + '.MOQMKP_PER', mode = 'a+', stamp = '---Generation '+ str(g)+' ----')
            # print_variables_to_file(self.solutions, 'VAR.' + algorithm_name + '.MOQMKP_PER', mode = 'a+')
            # front = get_non_dominated_solutions(self.solutions)
            # front_size = len(front)
            # print_stamp_to_file('FUN.' + algorithm_name + '.MOQMKP_PER', mode = 'a+', stamp = '---Generation '+ str(g)+' ----')
            # print_stamp_to_file('FUN.' + algorithm_name + '.MOQMKP_PER', mode = 'a+', stamp = 'front size: ' + str(front_size) + ' ----')
            # print_stamp_to_file('FUN.' + algorithm_name + '.MOQMKP_PER', mode = 'a+',
            #                     stamp = ' ')
            # print_function_values_to_file(self.solutions, 'FUN.' + algorithm_name  + '.MOQMKP_PER', mode = 'a+')
